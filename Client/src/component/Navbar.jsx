...
function Navbar ()  {
     ...
     const {User, setUser,api, setShowUserLogin,searchQuery, setSearchQuery,getCartCount} = userAppContext();

     const logout = async ()=>{
        try {
            const {data} = await api.get('http://localhost:4000/api/user/logout')
            if(data.success){
